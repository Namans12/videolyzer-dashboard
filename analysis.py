...
BRAVIA_8_II: dict[str, Any] = {
    ...
    # Sony TVs (including the Bravia 8 II) do NOT decode HDR10+ — a file
    # carrying HDR10+ metadata falls back to its HDR10 static base layer
    # for tone-mapping. We still *detect* HDR10+ via hdr10plus_tool so the
    # user knows it's present, but it must not be listed as a supported
    # display format and must not contribute to the TV-specific quality
    # score (see score_for_tv + composite_score guards downstream).
...
def run_mediainfo(file_path: str) -> dict[str, Any] | None:
    ...
    def _parse(args: list[str]) -> dict[str, Any] | None:
        result = run_command(args, timeout=MEDIAINFO_TIMEOUT_SECONDS)
        # MediaInfo emits non-zero exit on partial/truncated files even when
        # its JSON output is fully usable — so don't gate on returncode, only
        # on whether the stdout parses as JSON.
    ...
    # First try a deep parse (--ParseSpeed=1.0): it forces full SEI extraction
    # so mastering-display / MaxCLL / MaxFALL / HDR10+ profile / Atmos JOC and
    # HEVC-SPS bit depth all come through. On very large files (4K REMUX/WEB-DL
    # 30-40 GB+) the deep parse can exceed the timeout and return None — which
    # used to silently drop us to the ffprobe-only path, losing MediaInfo's
    # rich audio/HDR labels. So if the deep parse fails, retry at default
    # speed: on the iT/AMZN WEB-DL masters tested, the default parse still
    # returns full HDR10+ + MaxCLL + per-track audio, just faster.
    deep = _parse([mediainfo_bin, "--ParseSpeed=1.0", "--Output=JSON", file_path])
    if deep is not None:
        return deep
    logger.info("MediaInfo deep parse failed/timed out on %s — retrying at "
                 "default parse speed.", file_path)
    return _parse([mediainfo_bin, "--Output=JSON", file_path])
...
def get_container_short_name(container_name: str) -> str:
    """Map MediaInfo's verbose `Format` field to one of our short tokens.

    MediaInfo names are inconsistent across versions and file variants:
      - .mp4 → "QuickTime / MOV" (because MP4 is ISO BMFF / QuickTime-derived)
      - .mp4 → "MPEG-4" on some files
      - .mkv → "Matroska"
      - .m2ts → "BDAV" or "MPEG-TS"
    Without a wide-net match here, MP4 falls through to "Unknown", which
    poisons USB compatibility (false-positive issue) AND scoring (no +6 MP4
    bonus that the Bravia spec wants — MP4 is the recommended DV container).
    """
    text = lower_text(container_name)
    if "matroska" in text or "ebml" in text:
        return "MKV"
    if ("mp4" in text or "mpeg-4" in text
            or "quicktime" in text or "mov" in text or "isom" in text
            or "iso media" in text):
        return "MP4"
...
def estimate_video_bitrate_mbps(
    file_size_bytes: float,
    duration_min: float,
    audio_total_kbps: float = 0.0,
) -> float | None:
    """Estimate video bitrate as (overall ÷ duration) − audio − overhead.

    This replaces the old flat `× 0.85` heuristic. That assumed video is 85%
    of the file, which badly undershoots modern WEB-DL releases that carry a
    single ~768 kbps audio track over a ~24 Mbps video stream (video is ~97%
    of the file there, so 0.85 lost ~3 Mbps and dropped TV scores by ~11).

    When we know the real audio bitrate (the audio tracks live in the MKV/MP4
    header, so they're readable even from a partial magnet probe), subtract it
    directly. Fall back to a light 3% muxing-overhead haircut when audio is
    unknown — still far better than 15%.
    """
    if not file_size_bytes or not duration_min or duration_min <= 0:
        return None
    overall_mbps = (file_size_bytes * 8) / (duration_min * 60) / 1_000_000
    if audio_total_kbps > 0:
        # Known audio: subtract it plus a tiny subtitle/container allowance.
        video = overall_mbps - (audio_total_kbps / 1000.0) - 0.10
    else:
        # Unknown audio: assume ~3% container+audio overhead.
        video = overall_mbps * 0.97
    if video <= 0.5:
        return None
    return round(video, 2)


def sum_audio_bitrate_kbps(
    mediainfo_audio_tracks: list[dict[str, Any]],
    ffprobe_audio_streams: list[dict[str, Any]],
) -> float:
    """Total bitrate of all audio tracks in kbps (0 if unknown).

    Used to subtract real audio from the overall bitrate when estimating
    video bitrate for partial-file (magnet) probes. Sums across *all* tracks
    because every audio track contributes to file size — important for MULTi
    releases with several language tracks."""
    total = 0.0
    if mediainfo_audio_tracks:
        for t in mediainfo_audio_tracks:
            br = coerce_float(t.get("BitRate"))
            if br and br > 0:
                total += br / 1000.0      # bps → kbps
        if total > 0:
            return total
    # ffprobe fallback
    for s in ffprobe_audio_streams or []:
        br = coerce_float(s.get("bit_rate"))
        if br and br > 0:
            total += br / 1000.0
    return total
...
def get_bitrate_mbps(
    ...
    # Compute the size-based estimate up front so we can use it to *detect*
    # partial-file artifacts. When MediaInfo / ffprobe parse a sparse head
    # probe (magnet mode), they divide the head bytes they actually read by
    # the (correct) duration and report a 3–5× low bitrate. If the size-based
    # estimate is much higher than the reported one, prefer the estimate.
    estimated_mbps: float | None = None
    if file_size_gb and duration_min and duration_min > 0:
        estimated_mbps = estimate_video_bitrate_mbps(
            file_size_gb * 1_000_000_000, duration_min, audio_total_kbps,
        )
    ...
    for candidate in candidates:
        ...
        if value and value > 1_000:
            reported = round(value / 1_000_000, 2)
            # Partial-probe sanity check: if size+duration says ≥ 2× the
            # reported bitrate, the reported one came from a sparse head
            # slice. Trust the file-size math instead.
            if estimated_mbps and estimated_mbps >= reported * 2:
    ...
    if ffprobe_data:
        ...
        if value and value > 1_000:
            ...
            if estimated_mbps and estimated_mbps >= reported * 2:
                return estimated_mbps
            return reported
    # Final fallback: estimate from file size + duration
    if estimated_mbps is not None:
        return estimated_mbps
    return 0.0
...
def _mediainfo_audio_details(track: dict[str, Any]) -> tuple[str, str]:
    """Return (label, details) for one MediaInfo audio track."""
    title      = normalize_text(track.get("Title", ""))
    commercial = normalize_text(track.get("Format_Commercial_IfAny", track.get("Format", "")))
    bitrate    = coerce_float(track.get("BitRate"))
    channels   = normalize_text(track.get("ChannelLayout", track.get("ChannelPositions", "")))
...
def get_primary_audio_summary(
    ...
    """Select the audio track that best represents the release's capability.

    Previously this picked the *default* track. That under-represents MULTi
    releases whose default is a low-bitrate foreign-language AC3 while the
    English Atmos/DTS-X track sits later in the file (real example: a French
    "MULTi" release with FR AC3 384 as default + EN E-AC3 JOC Atmos 768 as
    track 3). Scoring the default penalised a file that actually carries
    excellent audio.

    New behaviour: score every track, keep the best, and append a hint when
    the best track isn't the default (you'll have to switch audio) or when
    several tracks exist. The quality score reflects the *best* track because
    that's what matters when judging whether a release is worth grabbing.
    """
    if mediainfo_audio_tracks:
        scored = []
        for i, t in enumerate(mediainfo_audio_tracks):
            label, details = _mediainfo_audio_details(t)
            score = get_audio_quality_score(label, details)
            is_default = lower_text(t.get("Default", "")) == "yes"
            scored.append((score, i, is_default, label, details, t))
        # Best track = highest quality score; tie-break toward the earlier
        # (usually primary) track for determinism.
        scored.sort(key=lambda s: (s[0], -s[1]), reverse=True)
        best_score, best_i, _best_default, best_label, best_details, _ = scored[0]

        # Was the default track a worse one? (e.g. foreign AC3 default,
        # English Atmos buried later.)
        default_entry = next((s for s in scored if s[2]), None)
        notes: list[str] = []
        n_tracks = len(mediainfo_audio_tracks)
        if n_tracks > 1:
            notes.append(f"{n_tracks} audio tracks")
        if default_entry is not None and default_entry[1] != best_i and default_entry[0] < best_score:
            def_lang = normalize_text(default_entry[5].get("Language", "")) or "default"
            notes.append(f"best track is not the default — switch from the {def_lang} track")
        details = best_details
        if notes:
            details = f"{details} ({'; '.join(notes)})"
        return best_label, details, best_score

    if ffprobe_audio_streams:
        # ffprobe fallback (taken when MediaInfo is unavailable or its deep
        # parse timed out on a large file). Same best-track logic, plus
        # default-track / language-mismatch detection so MULTi releases whose
        # default audio is a foreign-language low-bitrate track still get the
        # "you'll need to switch audio" note.
        def _ff(stream: dict[str, Any], idx: int):
            codec   = normalize_text(stream.get("codec_name", "audio")).upper()
            profile = normalize_text(stream.get("profile", ""))
            tag_atmos = "atmos" in lower_text(json.dumps(stream.get("tags", {})))
            bitrate = coerce_float(stream.get("bit_rate"))
            channels = normalize_text(stream.get("channel_layout", ""))
            bitrate_text = f"{round(bitrate / 1000):.0f} kbps" if bitrate else "unknown bitrate"
            details = codec
            if "atmos" in lower_text(profile) or tag_atmos:
                details = f"{details} Atmos"
            if channels:
                details = f"{details}, {channels}"
            details = f"{details}, {bitrate_text}"
            is_default = bool(stream.get("disposition", {}).get("default"))
            lang = lower_text(stream.get("tags", {}).get("language", ""))
            score = get_audio_quality_score(codec, details)
            return (score, idx, is_default, codec, details, lang)

        ranked = [_ff(s, i) for i, s in enumerate(ffprobe_audio_streams)]
        ranked.sort(key=lambda r: (r[0], -r[1]), reverse=True)
        best_score, best_i, _bd, best_codec, best_details, _bl = ranked[0]

        notes: list[str] = []
        n = len(ffprobe_audio_streams)
        if n > 1:
            notes.append(f"{n} audio tracks")
        default_entry = next((r for r in ranked if r[2]), None)
        if (default_entry is not None and default_entry[1] != best_i
                and default_entry[0] < best_score):
            def_lang = default_entry[5] or "default"
            notes.append(f"best track is not the default — switch from the {def_lang} track")

        details = best_details
        if notes:
            details = f"{details} ({'; '.join(notes)})"
        return best_codec, details, best_score
...
def check_usb_compatibility(
    ...
    # NOTE: previously emitted a per-file warning whenever file_size_gb > 3.9
    # ("use exFAT on your USB drive (FAT32 caps at ~4 GB)") and a paired tip
    # for smaller files. Removed — every UHD release trips it, so the warning
    # became noise that crowded out real compatibility issues. The same
    # guidance is still in the always-shown `BRAVIA_8_II["usb_notes"]` list
    # rendered as Drive Format / Tips in the UI.
...
def tv_compatibility_heuristic(
    ...
    # Sony Bravia 8 II doesn't decode HDR10+ — surface that explicitly when
    # the file carries it. The TV will tone-map from the HDR10 base layer,
    # so playback still works fine, just without per-scene dynamic metadata.
    hdr_lower = lower_text(hdr_summary)
    has_hdr10_plus = ("hdr10+" in hdr_lower or "smpte st 2094 app 4" in hdr_lower
                       or "smpte 2094-40" in hdr_lower)
    if has_hdr10_plus:
        note = (note + " HDR10+ present in stream but ignored by this TV "
                 "— playback falls back to the HDR10 base layer.").strip()
...
def build_tool_reports(
    ...
    hdr10_plus_scan: dict[str, Any] | None = None,
) -> list[dict[str, Any]]:
    ...
    if hdr10_plus_scan is not None:
        # Map our scan-specific statuses to the same vocabulary the other
        # tool reports use, so the UI badge renderer keeps working.
        status_map = {
            "confirmed":   "ok",
            "negative":    "ok",            # ran cleanly, just no HDR10+ found
            "unavailable": "unavailable",
            "error":       "error",
        }
        reports.append({
            "name":     "hdr10plus_tool",
            "status":   status_map.get(hdr10_plus_scan.get("status"), "unavailable"),
            "headline": hdr10_plus_scan.get("headline", "HDR10+ scan not available."),
            "details":  hdr10_plus_scan.get("details", []),
        })
    return reports
...
def analyze_file(filepath: str, skip_dovi_scan: bool = False,
    ...
    (magnet mode) but we know the true torrent file size, pass it here so the
    real size flows INTO bitrate + TV/quality scoring from the start. Patching
...
def _analyze_file_inner(filepath: str, skip_dovi_scan: bool = False,
    ...
    # ── HDR10+ confirmation (cheap, run even in fast mode) ─────────────────
    # We need the duration here so the multi-slice scanner can convert
    # middle-piece byte offsets to ffmpeg `-ss` time positions. The duration
    # is derived from MediaInfo/ffprobe data we already have, so this is a
    # cheap re-computation rather than an extra subprocess.
    duration_for_hdr_scan = get_duration_minutes(general_track, ffprobe_data) * 60.0

    # Imported lazily so analysis.py can be loaded without hdr10plus_scan
    # (avoids a circular import at module load and lets the rest of the
    # pipeline keep working if hdr10plus_scan itself is broken).
    hdr10_plus_scan: dict[str, Any] = {
        "status": "unavailable", "headline": "HDR10+ scan not attempted.",
        "details": [], "frames": 0, "confirmed": False,
    }
    try:
        # Only run when HEVC is present — HDR10+ via ST 2094-40 SEI is HEVC-only.
        codec_text = lower_text(
            (video_track.get("Format", "") if video_track else "")
            + " "
            + (ffprobe_video.get("codec_name", "") if ffprobe_video else "")
        )
        ...
        logger.warning("hdr10plus scan crashed on %s: %s", filepath, exc)
    ...
    # When a real-size override is supplied (magnet partial probe), trust it
    # over the probe file's own size — the probe is only a head slice.
    if size_override_bytes and size_override_bytes > 0:
        file_size_gb = round(size_override_bytes / 1_000_000_000, 2)
    else:
        file_size_gb = get_file_size_gb(general_track, ffprobe_data)
    duration_minutes = get_duration_minutes(general_track, ffprobe_data)
    # Total audio bitrate across all tracks — feeds the video-bitrate estimate
    # so partial-file probes subtract real audio instead of guessing 15%.
    audio_total_kbps = sum_audio_bitrate_kbps(mediainfo_audio_tracks, ffprobe_audio_streams)
    bitrate_mbps   = get_bitrate_mbps(video_track, ffprobe_video, ffprobe_data,
                                       ...
                                       audio_total_kbps=audio_total_kbps)
    ...
    tv_support = tv_compatibility_heuristic(
        ...
        # Pass the HDR string so the heuristic can add the "HDR10+ falls back
        # to HDR10 on this TV" note when relevant.
    ...
    return {
        ...
        "hdr":              dv_info["hdr"],
        # HDR10+ confirmation from hdr10plus_tool (independent of MediaInfo).
        # True if the tool found ≥1 SMPTE ST 2094-40 SEI frame in the scanned
        # bitstream. False on negative scan; None when scan unavailable/errored.
        "hdr10_plus_confirmed": (
            True if hdr10_plus_scan["status"] == "confirmed"
            else False if hdr10_plus_scan["status"] == "negative"
            else None
        ),
        "hdr10_plus_frames":   hdr10_plus_scan["frames"],
        "hdr10_plus_status":   hdr10_plus_scan["status"],
        "bitrate_mbps":     bitrate_mbps,
        "bit_depth":        bit_depth,
        ...
        "audio_details":    audio_details,
        "audio_score":      audio_score,
        # Total audio bitrate (all tracks) — used by magnet.py to recompute
        # video bitrate accurately when it patches in the real file size.
        "audio_total_kbps": round(audio_total_kbps, 1),
        "file_size_gb":     file_size_gb,
        # General quality
        "score":            quality_score,
        "confidence_score": confidence_score,
        ...
        "tool_reports":     build_tool_reports(
            ...
            hdr10_plus_scan=hdr10_plus_scan,
        ),
    }
